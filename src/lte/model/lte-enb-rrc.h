...
public:
  ...
  void SetEnbApp(Ptr<EpcEnbApplication> app);
  Ptr<EpcEnbApplication> GetEnbApp (void);

  uint16_t m_timer;
  std::map<uint16_t, uint16_t> m_timerStatus;
...
private:
  ...
  void Resend (EpcX2Sap::UeDataParams params, uint16_t rnti);
